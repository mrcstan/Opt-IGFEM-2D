...
#endif
{
    ...
    for (std::size_t ch = 0; ch < parent1.children.n_elem; ch++)
    {
                ...
                if (calcGrad)
                {
                    ...
                    for (std::size_t j = 0; j < parent1.vel.n_slices; j++)
                    {
                        ...
                        gradObjRemEl(parent1.designParamNum(j))
                            += (pow(uh,objOpt.normp)*divv+objOpt.normp*pow(uh,objOpt.normp-1)*arma::dot(UURel,D2N))*factor;                            
                        //gradObjRemEl(parent1.designParamNum(j)) 
        ...
        //       if the sensitivity is not needed!                    // 
        ////////////////////////////////////////////////////////////////
        if (!calcGrad) continue;
