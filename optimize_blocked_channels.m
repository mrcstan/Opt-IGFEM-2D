...
%   blockedChannelFile:
%        file containing information about the sets of blocked channels
%   jobNum_sampleNum:
...
%   costFuncType:
%       i) P_NORM
%       ii) NODAL_T_IN, NODAL_T_OUT or NODAL_T_INOUT
%   costFuncNormp
%   nlconType:
%       i) Pmax
%       ii) Pmin
...
function  optimize_blocked_channels(channelFile,...
                                    polygonFile,...
                                    boundsFile_sampleFile,...
                                    blockedChannelFile,...
                                    jobNum_sampleNum,...
...
sensitivity.options ...
    = optimoptions(@fmincon, ...
        ...
        'FinDiffType','central', ...
        'outputfcn', @outfun_blocked_channels,...  % output the history of the converged solution
        'Algorithm', 'sqp',...  % Choose the algorithm to solve the optimization
        'TolCon', 1e-6,...   % tolerance for constraint
        'TolX',1e-7,...      % tolerance for parameter value (multiplied by velocity) 
...
fprintf(fidSim,'parameters for minimax optimization %i\n',nSimulations);
...
fprintf(fidSim,'blocked-channel file %s\n',blockedChannelFile);
...
% read channel numbers to be blocked
blockedSets = read_blocked_channels(blockedChannelFile);
G_history.nMaxObjs = numel(blockedSets);
...
for simNum = 1:nSimulations
    ...
    try 
        ...
        totParams = sensitivity.designParams.nParams ...
            +sensitivity.restrictedParams.nParams + 1; % additional constraint is needed for minimax problem 
        ...
        [Aeq,beq] = linear_equality_constraints(sensitivity.restrictedParams, ...
                                                sensitivity.designParams.nParams+1);

        % initial guess for the change in design parameter
        iniGuess = zeros(totParams,1); 
        ...
        if (scaleObj) 
            ...
            sensitivity.objScale = abs(fem_n_sensitivity (iniGuess(1:end-1),  ...
                                                      mesh, ...
                                                      channels, ...
                                                      femParams, ...
                                                      sensitivity, ...
            ...
            costfun = @ (delParams) minimax_param(delParams,sensitivity.objScale);
            iniGuess(end) = sensitivity.objScale;
        else
            G_out.objScale = 1.0;
            costfun = @ (delParams) minimax_param(delParams,1);
        end
                        
        nlconfun = @ (delParams) nonlinear_constraints_blocked_channels(...
                                               delParams, ...
                                               blockedSets, ...
                                               sensitivity.designParams, ...
                                               sensitivity.restrictedParams, ...
